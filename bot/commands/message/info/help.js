...
module.exports = {
	...
	run: async function(message, client, args) {
		...
		if (!args.command) {
			...
			let paginatorEmbeds = [];
			let modules = fs.readdirSync("./bot/commands/message");
			for (let i = 0; i < modules.length; i++) {
				let upperCaseModule = modules[i].charAt(0).toUpperCase() + modules[i].slice(1);
				let commandsInModule = fs.readdirSync(`./bot/commands/message/${modules[i]}`).filter(file => file.endsWith(".js"));
				...
				if (commandsInModule.length > 10) {
					for (let j = 0; j < commandsInModule.length; j += 10) {
						...
						if (commandsInModule.length > 0) {
							for (let k = j; k < j + 10 && k < commandsInModule.length; k++) {
								let command = require(`../../message/${modules[i]}/${commandsInModule[k]}`);
					...
					if (commandsInModule.length > 0) {
						for (let j = 0; j < commandsInModule.length; j++) {
							let command = require(`../../message/${modules[i]}/${commandsInModule[j]}`);
							moduleEmbed.addFields([{
								name: `\`${serverPrefix}${command.name}\``,
								value: command.description,
								inline: false
